...
export default function TourismTab({ D }) {
  ...
  return (
    <div className="tab-pane">
      <div className="section-title">{t.tourismTitle}</div>
      <ResponsiveContainer width="100%" height={190}>
        <ComposedChart data={data} margin={{top:4,right:8,bottom:0,left:-14}}>
          ...
          <Bar  yAxisId="l" dataKey="litter"   name={t.tourBeachLitter} fill="rgba(248,113,113,.6)"  radius={[3,3,0,0]}/>
          <Line yAxisId="r" dataKey="tourists" name={t.tourTourists}    stroke="var(--accent2)" strokeWidth={2} dot={{r:3,fill:'var(--accent2)'}} type="monotone"/>
        </ComposedChart>
      </ResponsiveContainer>
      <div className="note">
        Pearson r = <span className="pos">−0.44</span> · {t.tourNote1}<br/>
        <span style={{fontSize:9}}>{t.tourNote2}</span>
      </div>
    </div>
  );
}
