...
\usepackage{enumitem}
\usepackage{xcolor}
% titlesec and hyperref removed for BasicTeX compatibility
% \usepackage{titlesec}
...
\author{Daniele Uras \quad\textbar\quad Filippo Saccomano \quad\textbar\quad Tommaso Nesa\\[2pt]
Tommaso Del Vecchio \quad\textbar\quad Viola Guazzoni\\[2pt]
...
is an open, reproducible pipeline that processes six monitored data compartments and combines the
...
The project was carried out by a five-person team. We worked on a shared GitHub repository
(\texttt{daniele22-u/hhe-labsardinia}) using feature branches and frequent, descriptive commits, so
that every analytical decision is traceable in the version history. All team members contributed
across different parts of the project --- data engineering, spatial analysis, exploratory analysis,
biological data processing, and communication. Daniele Uras and Filippo Saccomano led the
development of the interactive dashboard and the React/Leaflet front-end. Design decisions that
shape the whole output (compartment weights, grid resolution, validation strategy) were made
jointly by the full team.
...
contract removed a classic multi-person failure mode --- divergent intermediate CSVs --- because the
JSON schema is the only interface the work-streams share. Adding a compartment therefore means
...
\emph{hazard}. We therefore collected and analysed six compartments, then made an explicit choice
...
\end{itemize}
Excluding these is not an omission but a guard: adding driver-type or temporally incomplete
signals would weaken, not strengthen, the index. The dashboard still exposes all six as
...
\item \textbf{Spatial statistics.} A visual hotspot is not evidence. We added Global
...
\texttt{03\_spatial\_hazard}\\
Integration & Single canonical dataset assembled from all compartments, exported to JSON &
\texttt{00\_analysis} $\rightarrow$ \texttt{dashboard\_data.json}\\
...
repository ships a README documenting environment setup. The full analysis pipeline runs in a single
self-contained notebook (\texttt{00\_analysis.ipynb}); modular variants are available as
\texttt{01\_data\_cleaning}, \texttt{02\_eda}, and \texttt{03\_spatial\_hazard} for step-by-step
inspection. All paths are relative to the repository root. The dashboard is a static build served
from the generated JSON, so it needs no backend. A clean checkout therefore regenerates every figure
and the full dashboard from the raw files with no manual editing --- the main usability risk that
remains is obtaining the raw ISPRA modules, which require manual download from the national portal
...
the only excluded compartment that is a genuine hazard state. \emph{(ii)} Move from description to explanation with \textbf{spatial regression}
